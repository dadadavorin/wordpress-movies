 ...
 *
 * @package Movies\AdminMenus
 */

declare(strict_types=1);

namespace Movies\AdminMenus;

use MoviesVendor\EightshiftLibs\AdminMenus\AbstractAdminMenu;
...
class ReusableBlocksHeaderFooter extends AbstractAdminMenu
{
	...
	{
		return \esc_html__('Header & footer', 'Movies');
	}
	...
	public function renderPartialSelector($args): void
	{
		...
		if ($reusableBlocksQuery->have_posts()) {
			...
			<select id="<?php echo \esc_attr($args['label_for']); ?>" name="<?php echo \esc_attr($type); ?>">
				<option value="">
					&mdash; <?php \esc_html_e('None', 'Movies'); ?> &mdash;
				</option>
		...
		<?php } else { ?>
			<i><?php echo \esc_html__('No reusable blocks found.', 'Movies'); ?></i>
			<?php
		}
	}
