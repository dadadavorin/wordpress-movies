...
export const ImageOptions = (attributes) => {
	...
	if (!hideImagePicker && !imageUrl[breakpoints[0]]) {
		return (
			<UseToggle {...generateUseToggleConfig(attributes, manifest, 'imageUse')}>
				<Control icon={icons.image} label={__('Image', 'Movies')} additionalLabelClasses='es-h-spaced' noBottomSpacing>
					<MediaPlaceholder
						labels={{
							title: __('Add an image', 'Movies'),
	...
	return (
		<UseToggle {...generateUseToggleConfig(attributes, manifest, 'imageUse')}>
			{!hideImagePicker &&
				<Responsive label={__('Image', 'Movies')} icon={icons.image}>
			...
			>
				...
				{!hideFullSizeToggle &&
					<IconToggle
						icon={icons.expandXl}
						label={__('Fill container', 'Movies')}
						checked={imageFull}
			...
			<Section
				showIf={!hideAltText}
				icon={icons.a11y}
				label={__('Accessibility', 'Movies')}
				noBottomSpacing={!additionalControlsAfterA11y}
			>
				<TextControl
					label={<IconLabel icon={icons.altText} label={__('Alt text', 'Movies')} />}
					value={imageAlt}
					onChange={(value) => setAttributes({ [getAttrKey('imageAlt', attributes, manifest)]: value })}
					help={__('Describes the content of the image', 'Movies')}
