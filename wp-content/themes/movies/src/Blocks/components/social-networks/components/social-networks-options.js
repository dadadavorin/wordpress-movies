...
export const SocialNetworksOptions = (attributes) => {
	...
	const modeOptions = [
		{
			label: __('Link to social networks', 'Movies'),
			icon: icons.link,
			value: false,
		},
		{
			label: __('Share to social networks', 'Movies'),
			icon: icons.share,
			value: true,
		}
	];

	const pickerSectionTitle = socialNetworksShareMode ? __('Share targets', 'Movies') : __('Social networks', 'Movies');
