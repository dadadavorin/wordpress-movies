...
export const WrapperOptions = ({ attributes, setAttributes }) => {
	const {
		label = __('Spacing & layout', 'Movies'),
		showWrapperId = true,
	...
	let wrapperUseOptions = [
		{
			label: __('Just the block', 'Movies'),
			value: 'off',
					...
					{!wrapperNoControls && wrapperUseShowControl &&
						<div className='es-display-flex es-items-center es-gap-1 es-ml-auto es-flex-shrink-0 es-mr-1 es-wrapper-options-copy-paste'>
							<Button
								...
								label={__('Copy configuration', 'Movies')}
								showTooltip
							/>
							...
							<Button
								...
								className='es-button-icon-24 es-button-square-24 es-rounded-0.75!'
								label={__('Paste configuration', 'Movies')}
								showTooltip
							/>
						</div>
					}
				...
				<>
					...
					>
						...
						{(!noLeftSpacingInSelector || !noRightSpacingSelector) &&
							<Collapsable label={__('More', 'Movies')} icon={icons.moreH}>
								{!noLeftSpacingSelector &&
									<ResponsiveNumberPicker
										icon={icons.spacingLeft}
										label={__('Left', 'Movies')}
						...
						{(!noLeftSpacingInSelector || !noRightSpacingInSelector) &&
							<Collapsable label={__('More', 'Movies')} icon={icons.moreH} noBottomSpacing>
								{!noLeftSpacingInSelector &&
									<ResponsiveNumberPicker
										icon={icons.spacingLeftIn}
										label={__('Left inner', 'Movies')}
					...
					>
						{showWrapperBgColorPicker &&
							<AdvancedColorPicker
								...
								types={
									[
										{
											label: __('None', 'Movies'),
											value: '',
											icon: icons.emptyCircle,
										},
										{
											label: __('Project color', 'Movies'),
											value: 'project',
											icon: icons.colorAlt,
										},
										{
											label: __('Gradient', 'Movies'),
											value: 'gradient',
											icon: icons.gradient,
										}
									]
						...
						{showWrapperRoundedCorners &&
							<ResponsiveNumberPicker
								...
								label={__('Rounded corners', 'Movies')}
						...
						{(showWrapperDividerTop || showWrapperDividerBottom || showWrapperDividerLeft || showWrapperDividerRight || showWrapperDividerColor) &&
							...
							>
								{showWrapperDividerColor &&
									<ColorPicker
										colors={[{ name: __('None', 'Movies'), slug: undefined, color: 'transparent' }, ...dividerColors]}
										...
										pickerPopupTitle={__('Divider color', 'Movies')}
											...
											{breakpointNames.map((breakpoint, index) => {
												...
												return (
													<Button
														...
														label={attributeValue ? __('On', 'Movies') : __('Off', 'Movies')}
														showTooltip
													/>
												);
											})}
					...
					<Section
						...
						icon={icons.tools}
						label={__('Advanced', 'Movies')}
						subtitle={__('Visibility, anchor, ID', 'Movies')}
						noBottomSpacing
						collapsable
					>
						{showWrapperHide &&
						...
						{showWrapperAnchorId &&
							<TextControl
								label={<IconLabel icon={icons.anchor} label={__('Block Anchor ID', 'Movies')} />}
						...
						{showWrapperId &&
							<TextControl
								label={<IconLabel icon={icons.id} label={__('Block unique identifier', 'Movies')} />}
								value={wrapperId}
